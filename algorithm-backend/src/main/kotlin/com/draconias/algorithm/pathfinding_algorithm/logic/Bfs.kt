...
class Bfs: Pathfinder {
    ...
    private val directions = listOf(
        Pair(-1, 0), Pair(1, 0), Pair(0, -1), Pair(0, 1), Pair(-1, -1), Pair(-1, 1), Pair(1, -1), Pair(1, 1)
    )

    override suspend fun findPath(gridCell: List<List<GridCell>>, startNode: GridCell) {
        ...
        while(queue.isNotEmpty()){
            ...
            for (direction in directions){
                val newX = cell.col + direction.first
                val newY = cell.row + direction.second
